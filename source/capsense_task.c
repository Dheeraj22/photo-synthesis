...
QueueHandle_t capsense_touch_q;
...
static void process_touch(void)
{
    ...
    /* Variable used for sending touch queue */
    bool send_touchq = false;
    capsense_touch_t touchinfo = {false,false,false,false,0u};
    ...
    slider_gesture = Cy_CapSense_DecodeWidgetGestures(
	        CY_CAPSENSE_LINEARSLIDER0_WDGT_ID,
	        &cy_capsense_context);
    ...
    /* Slider right flick */
	if(slider_gesture == 0x2004080)
	{
        touchinfo.sldr_right = true;
        send_touchq = true;

		printf("SLDR flick right \r\n\n");
	}
	/* Slider left flick */
	else if (slider_gesture == 0x3004080)
	{
        touchinfo.sldr_left = true;
        send_touchq = true;

		printf("SLDR flick left \r\n\n");
	}
	/* Slider long press */
	else if (slider_gesture == 0x400)
	{
		if (slider_pos_perv != slider_pos )
		{
	        touchinfo.slider_val = slider_pos;
	        send_touchq = true;
    ...
    if(send_touchq)
    {
        xQueueSendToBack(capsense_touch_q, &touchinfo, 0u);
    }
...
static void capsense_timer_callback(TimerHandle_t xTimer)
{
    ...
    gestureTimeStamp += CAPSENSE_SCAN_INTERVAL_MS;
